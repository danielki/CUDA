...
void openDisplay(int h, int w, int argc, char **argv, rgb* pixelarray)
    ...
    for ( int y=0; y < height; y++ )
        ...
        for ( int x=0; x < width; x++ )
            ...
            image[3*(y*width + x)    ]=pixelarray[(height-1-y)*width + x].r / 255.0;
            image[3*(y*width + x) + 1]=pixelarray[(height-1-y)*width + x].g / 255.0;
            image[3*(y*width + x) + 2]=pixelarray[(height-1-y)*width + x].b / 255.0;
            }
        }
