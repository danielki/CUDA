...
struct  deviceData
 ...
 __device__ __host__ rgb shade(const ray &r, const triangle &t)
    ...
    rgb shadeColor;
    shadeColor.x = winkel * t.farbe.x;
    shadeColor.y = winkel * t.farbe.y;
    shadeColor.z = winkel * t.farbe.z;
    return shadeColor;
    }
...
__global__ void renderImage(triangle* prims, deviceData* devDat, rgb* image, point* lights) 
	...
	if ( (t.A.x != INT_MAX) && (x < deviceDataShared->width) && (y < deviceDataShared->height) )
		...
		if ( shadow < 1 )	
			...
			farbe.x = shadow * farbe.x;
			farbe.y = shadow * farbe.y;
			farbe.z = shadow * farbe.z;
			}	
