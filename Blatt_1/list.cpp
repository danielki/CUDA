...
void removeElementAtPosition(list *l, int position)
    {
    /* Wenn position kleiner als 1 ( größer als
    lenght()) wird erstes (letztes) element
    entfernt*/
    if ( lenght(l) == 0 )
        {
        return;
    ...
    if ( position <= 1 )
        {
        listElement *lefree = l->firstElement;
...
void reverseOrder(list *l)
    ...
    /* Dreht alle Zeiger um und lässt
    Listenanfang auf das letzte Element zeigen */
    if (lenght(l) > 2 )
        {
        listElement *le = l->firstElement;
