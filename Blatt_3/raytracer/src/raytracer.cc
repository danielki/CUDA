...
void render_image(const scene& s, const int& height, const int& width, rgb* image) {
	...
	point mittelPunkt;
    ray camrichtung;
    camrichtung.start = s.cam.position;
    camrichtung.richtung = s.cam.richtung;
    // mittlerer schnittpunkt mit der Bildebene
	intersect(camrichtung,s.cam.obenLinks,camrichtung.richtung,mittelPunkt);
    // richtungsvektor rechts
	point right = cross(s.cam.oben,camrichtung.richtung);
	point obenMitte;
    ray rechtsVonObenLinks;
	// Strahl definieren für intersection
    rechtsVonObenLinks.start = s.cam.obenLinks;
    rechtsVonObenLinks.richtung = right;
    // schnittpunkt oben mitte
	intersect(rechtsVonObenLinks,mittelPunkt,right,obenMitte);
    // standart schrittweite für x // y wert
